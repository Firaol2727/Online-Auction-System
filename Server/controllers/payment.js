...
const paychapa=async (aid,req,res)=>{
    ...
    let user2=await Seller.findOne({where:{
        id:bid
    }})
    if(user){
        console.log("User found")
        var myHeaders = new fetch.Headers();
        myHeaders.append("Authorization", "Bearer CHASECK_TEST-ulpl5C8atPD1UqnYqrKifXF6D4qUgZli");
        ...
        var raw = JSON.stringify({
            ...
            "tx_ref":a,
            "callback_url": "https://webhook.site/077164d6-29cb-40df-ba29-8a00e59a7e60",
            "return_url": `http://localhost:5173/sel/newauction`,
            "customization[title]": "Payment for the auction",
            "customization[description]": "I want to participate on Nuchereta Auctions"
        });
        var requestOptions = {
...
const verifyOrders=async(order)=>{
    ...
    try {
        ...
        return axios.get(`https://api.chapa.co/v1/transaction/verify/${tx_ref}`,
        { headers }
        ).then(async(data)=>{
            ...
            if (verifySession.status === "success") {
                ...
                let paidamount=verifySession.data.amount==null?0:verifySession.data.amount;
...
const verifyOrdersSeller=async(order)=>{
    let verify_success=false;
    try {
        if (!order.textref) {
                console.log("No text ref")
        ...
        { headers }
            ...
            if (verifySession.status === "success") {
                ...
                let paidamount=verifySession.data.amount==null?0:verifySession.data.amount;
                let prev_amount=Number(seller.account);
                console.log("The prev account",prev_amount);
                console.log("The paid amount",paidamount)
                let newamount=prev_amount+paidamount;
                console.log("THe new amount is ",newamount)
...
const chapaVerifySeller = async (userid) => {
    let haspaid=false;
    let unverifiedorders=await Order.findOne({
        where:{
            userid:userid,
    ...
    console.log("There are unverified orders",unverifiedorders)
    if(unverifiedorders){
        let result=await verifyOrdersSeller(unverifiedorders)
        console.log("The result from verifing orders is ",result);
        if(result==true){
            haspaid=true;
        }
